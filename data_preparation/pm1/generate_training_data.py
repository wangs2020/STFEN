...
def generate_data(args: argparse.Namespace):
    ...
    # read data
    df = pd.read_csv(data_file_path,usecols=fea_lt + ['st_ID', 'date'])#[:50000]
    #df_index = pd.to_datetime(df["time"].values, format="%Y/%m/%d").to_numpy()
    df['date'] = pd.to_datetime(df['date'].values)
    print(df.info())

    #df = df.loc[df[label] > 0,:]
    df = df.sort_values(by=['st_ID','date'],ascending=True).reset_index()
    ...
    print(data.shape)
    data = data[..., label_channel]
    print("raw time series shape: {0}".format(data.shape))

    index_list = []
    skip_len = 0
    num_samples = len(data) - (history_seq_len + future_seq_len) + 1
    ...
    df = df.reset_index()
    df['img_path'] = img_dir + "/" + df['st_ID'].astype(str) + ".nc"
    img_paths = {}
...
if __name__ == "__main__":
    # sliding window size for generating history sequence and label sequence
    HISTORY_SEQ_LEN = 0
    FUTURE_SEQ_LEN = 1
    YEAR = 2021
    ...
    TRAIN_RATIO = 0.7
    VALID_RATIO = 0.1
    TARGET_CHANNEL = [0]                   # label channel(s)
    STEPS_PER_DAY = 1          # every 1 hour

    DATASET_NAME = "pm1"      # sampling frequency: every 1 hour
    TOD = False                  # if add time_of_day feature
    DOW = True                  # if add day_of_week feature
    DOM = True                  # if add day_of_month feature
    DOY = True                  # if add day_of_year feature

    #OUTPUT_DIR = "datasets/" + DATASET_NAME + "_mini"
    OUTPUT_DIR = "datasets/" + DATASET_NAME
    IMG_DIR = "/data6/wangs/pm1/dl/img_data_w40_concat/"
    DATA_FILE_PATH = "/data6/wangs/pm1/stfen/datasets/raw_data/{0}/{0}_{1}_daily.csv".format(DATASET_NAME,YEAR)
    FEATURE_LT = [
    ...
    parser.add_argument("--doy", type=bool, default=DOY,
                        help="Add feature day_of_week.")
    parser.add_argument("--label_channel", type=list,
