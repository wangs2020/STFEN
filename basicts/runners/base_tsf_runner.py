...
class BaseTimeSeriesForecastingRunner(BaseRunner):
    ...
    def __init__(self, cfg: Dict):
        ...
        self.scaler = load_pkl("{0}/scaler_in_{1}_out_{2}_rescale_{3}.pkl".format(
            ...
            cfg.get("RESCALE", False)))

        # generate scaler for target
        self.scaler['args_target'] = copy.deepcopy(self.scaler['args'])
        self.scaler['args_target']['mean'] = self.scaler['args']['mean'][:,-1,:]
        self.scaler['args_target']['std'] = self.scaler['args']['std'][:, -1, :]

        self.scaler['args']['mean'] = self.scaler['args']['mean'][:,:-1,:]
        self.scaler['args']['std'] = self.scaler['args']['std'][:, :-1, :]
    ...
    def build_train_dataset(self, cfg: Dict):
        ...
        index_file_path = "{0}/index_in_{1}_out_{2}_rescale_{3}.pkl".format(
            ...
            cfg["DATASET_OUTPUT_LEN"],
            cfg.get("RESCALE", False))
        img_file_name = "{0}/img_path_in_{1}_out_{2}_rescale_{3}.pkl".format(
            cfg["TRAIN"]["DATA"]["DIR"],
            cfg["DATASET_INPUT_LEN"],
            cfg["DATASET_OUTPUT_LEN"],
            cfg.get("RESCALE", False))
        ...
        dataset_args["img_file_name"] = img_file_name
        dataset_args["img_file_dir"] = cfg['IMG_DIR']
    ...
    def metric_forward(self, metric_func, args) -> torch.Tensor:
        ...
        # select single target to calculate metric
        # {"prediction": pred, "target": real}
    ...
    def rescale_data(self, input_data: Dict) -> Dict:
        ...
        if self.if_rescale:
            ...
            input_data["prediction"] = SCALER_REGISTRY.get(self.scaler["func"])(input_data["prediction"], **self.scaler["args_target"])#[:, :, -1, :]
            input_data["target"] = SCALER_REGISTRY.get(self.scaler["func"])(input_data["target"], **self.scaler["args_target"])#[:, :, -1, :]
        return input_data
    ...
    def train_iters(self, epoch: int, iter_index: int, data: Union[torch.Tensor, Tuple]) -> torch.Tensor:
        ...
        #print(forward_return["prediction"])
        #print(forward_return["target"])
        # loss
        if self.cl_param:
